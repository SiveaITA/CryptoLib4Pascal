...
function TSecP521R1Curve.CreateCTMultiplier(const ARandom: ISecureRandom; ABlindBits: Int32): IECMultiplier;
var
  LCurve: IECCurve;
  LFieldOps: IFpFieldOps;
begin
  LCurve := Self as IECCurve;
  LFieldOps := TSecP521R1FpFieldOps.Create(LCurve.A, LCurve.B, LCurve.Order);
  Result := TFpCTMultiplier<TSecP521R1FieldArith>.Create(LFieldOps, ARandom, ABlindBits);
end;

function TSecP521R1Curve.CreateBasePointCTMultiplier(const ARandom: ISecureRandom): IECMultiplier;
var
  LCurve: IECCurve;
  LFieldOps: IFpFieldOps;
begin
  LCurve := Self as IECCurve;
  LFieldOps := TSecP521R1FpFieldOps.Create(LCurve.A, LCurve.B, LCurve.Order);
  Result := TFpAffineCombMultiplier<TSecP521R1FieldArith>.Create(LFieldOps, ARandom);
end;
